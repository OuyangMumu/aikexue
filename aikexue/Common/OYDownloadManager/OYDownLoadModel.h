...
// 下载状态
typedef  NS_ENUM(NSUInteger, OYDownloadState) {
    OYDownloadStateNone,        // 未下载
    OYDownloadStateReadying,    // 等待下载
    OYDownloadStateRunning,     // 正在下载
    OYDownloadStateSuspended,   // 下载暂停
    OYDownloadStateCompleted,   // 下载完成
    OYDownloadStateFailed       // 下载失败
};

@class OYDownloadProgress;
@class OYDownloadModel;

// 进度更新block
typedef void (^OYDownloadProgressBlock)(OYDownloadProgress *progress);
// 状态更新block
typedef void (^OYDownloadStateBlock)(OYDownloadState state,NSString *filePath, NSError *error);

/**
 *  下载模型
 */
@interface OYDownloadModel : NSObject
...
// >>>>>>>>>>>>>>>>>>>>>>>>>>  task info
// 下载状态
@property (nonatomic, assign, readonly) OYDownloadState state;
// 下载任务
@property (nonatomic, strong, readonly) NSURLSessionTask *task;
// 文件流
@property (nonatomic, strong, readonly) NSOutputStream *stream;
// 下载进度
@property (nonatomic, strong ,readonly) OYDownloadProgress *progress;
// 下载路径 如果设置了downloadDirectory，文件下载完成后会移动到这个目录，否则，在manager默认cache目录里
@property (nonatomic, strong, readonly) NSString *filePath;

// >>>>>>>>>>>>>>>>>>>>>>>>>>  download block
// 下载进度更新block
@property (nonatomic, copy) OYDownloadProgressBlock progressBlock;
// 下载状态更新block
@property (nonatomic, copy) OYDownloadStateBlock stateBlock;
 ...
 *  下载进度
 */
@interface OYDownloadProgress : NSObject
