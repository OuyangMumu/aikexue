...
#import "OYDownloadSessionManager.h"
...
@interface OYDownloadSessionManager ()
...
@implementation OYDownloadSessionManager
...
{
    if (self = [super init]) {
        _backgroundConfigure = @"OYDownloadSessionManager.backgroundConfigure";
        _maxDownloadCount = 1;
...
- (NSString *)downloadDirectory
{
    if (!_downloadDirectory) {
        _downloadDirectory = [[NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) lastObject] stringByAppendingPathComponent:@"OYDownloadCache"];
...
// 取消下载 是否删除resumeData
- (void)cancleWithDownloadModel:(OYDownloadModel *)downloadModel clearResumeData:(BOOL)clearResumeData
{
...
// 配置后台后台下载session
- (void)configirebackgroundSessionTasksWithDownloadModel:(OYDownloadModel *)downloadModel
{
    if (!_backgroundConfigure) {
        return ;
    }
...
// 获取resumeData
- (NSData *)resumeDataFromFileWithDownloadModel:(OYDownloadModel *)downloadModel
{
...
{
    OYDownloadModel *downloadModel = [self downLoadingModelForURLString:downloadTask.taskDescription];
