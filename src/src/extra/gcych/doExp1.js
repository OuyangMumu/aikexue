...
var doExp1 = myLayer.extend({
  ...
  initUI:function(){
        ...
        this.toolbtn = createTool({
                        ...
                        for(var i = 17;i>=0;i--){ 
                            if(self.uiname[i]!="nie1" && self.uiname[i]!="nie2" 
                                && self.uiname[i]!="sg"){
                                  ...
                                  var itemchild = out_item.getChildren()[0]
                                  var itempos = cc.p(item.x-item.width/2,item.y-item.height/2)
                                  var pos = itemchild.convertToNodeSpace(itempos)
                                  var itemrect = cc.rect(0,0,itemchild.width,itemchild.height)
                                  
                                  if(cc.rectContainsPoint(itemrect,pos)){
            ...
            outfun:function(){
              if(self.curcutnum==15 && self.show){
                  self.show = false
                  self.nodebs.say({
                    key: "wenzi3",
                    force:true
