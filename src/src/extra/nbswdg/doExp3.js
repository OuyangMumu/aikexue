var doExp3 = myLayer.extend({
        ...
        this.expCtor({
            ...
            settingData: {
                ...
                biaogeFun:function(){
                    if(!self.biaoge){
                        var bg = createBiaoge({
                            json: res.nbswdg_tableNode_3_json,
                            inputNum:24,
                            scale:0.9,
                            judgeOrder:true,
                        })
    ...
    initFunlist: function() {
        ...
        this.startFunlist = [null,null,null,null]
        self.moveFunList = [move,move,move,move]
        var end2 = function(){
           this.removeListen()
        }
        this.endFunList = [null,null,null,end2]
        this.cupInfo = [{
                flagname: res.biaoqian1,
                levelPosY: 85,  //没倒水的时候液面刻度值
                levelPosY2: 122,  //倒水后刻度值
    ...
    createTool:function(){
        ...
        loadPlist("biaoqian3_ds3_plist")
        //创建一个烧杯用来倒水
        var shaobei = new cc.Sprite("#biaoqian3_ds301.png")
            ...
            var checkFun = function(sb){
                if(!sb.haveWater && self.checkDistance(item,sb)){
                    if(!item.haveWdj && !sb.haveWdj){
                        ...
                        if(sb.index == 0){
                            sb.tempwendu = ((2*sb.tempwendu+item.tempwendu)/3) * (Math.pow(((-8/672655)*dis),2)+(-13/177844)*dis+1)
                        }else{
                        ...
                        var frame = "biaoqian3_ds3%02d.png"
                        if(item.haveWater){
                        ...
                        shaobei.runAction(cc.sequence(
                            ...
                            cc.callFunc(function(){
                                ...
                                sb.levelValue = sb.levelValue + 25
                                if(item.haveWater){
                                    item.setPositionY(-600)
                                    item.forceBack(true)
                                    allSbBack = true
                                }else{
                                    item.level.setPositionY(45)
                                    item.haveWater = true
