var seeExp4 = myLayer.extend({
    ...
    initUI:function(){
        ...
        var node = loadNode(res.kc_seeExp4_json, uiList)
        self.inside_node.addChild(node)

        loadPlist("see4_plist")
        ...
        var judgeList = []
        var wenziList = []
        var local = [1,5,3,4,6,8,7]
        for(var i = 0 ; i < 7 ; i++){
            wenziList[i] = null
            var name = sprintf("#see4_judge_%d.png", i+1)
        ...
        createSp("#see4_title.png",cc.p(600,590),self)
        var bigImg_kou = createSp("#bigImg_chu.png",cc.p(455,335),self)
        bigImg_kou.setLocalZOrder(10)
        bigImg_kou.setVisible(false)
        var bigImg_chu = createSp("#bigImg_kou.png",cc.p(370,345),self)
        bigImg_chu.setLocalZOrder(10)
        bigImg_chu.setVisible(false)
        self.nodebs.show()
        node.curIndex = 10
        for(var i = 0 ; i < 7 ; i++){
        ...
        node.callFun = function(index){
            ...
            for(var i = 0 ; i < 7 ; i++){
                ...
                if(index == i){
                    ...
                    bigImg_chu.setVisible(false)
                    if(index == 5)
                        bigImg_kou.setVisible(true)
                    if(index == 6)
                        bigImg_chu.setVisible(true)
                    if(!select){
                        var name = sprintf("#see4_select_%d.png", i+1)
                        selectList[i] = createSp(name,node[uiList[7+i]].getPosition(),self)
                    }
                    if(!wenzi){
                        var name = sprintf("#see4_wenzi_%d.png", i+1)
    ...
    initPeople: function() {
        ...
        var addList = [
            {sound:res.see4_sound1},{sound:res.see1_sound7},{sound:res.see1_sound5},
            {sound:res.see4_sound4},{sound:res.see1_sound1},{sound:res.see4_sound3},
            {sound:res.see4_sound2}
        ]
