var seeExp2 = myLayer.extend({
    ...
    initUI:function(){
        ...
        var node = loadNode(res.kc_seeExp2_json, uiList)
        ...
        var judgeList = []
        var wenziList = []
        var local = [6,7,5,4,3,3,4]
        for(var i = 0 ; i < 7 ; i++){
            wenziList[i] = null
            var name = sprintf("#see2_judge_%d.png", i+1)
        ...
        createSp("#see2_title.png",cc.p(600,590),self)
        self.nodebs.show()
        node.curIndex = 10
        for(var i = 0 ; i < 7 ; i++){
        ...
        node.callFun = function(index){
            ...
            for(var i = 0 ; i < 7 ; i++){
                ...
                if(index == i){
                    ...
                    if(!select){
                        var name = sprintf("#see2_select_%d.png", i+1)
                        selectList[i] = createSp(name,node[uiList[7+i]].getPosition(),self)
                    }
                    if(!wenzi){
                        var name = sprintf("#see2_wenzi_%d.png", i+1)
    ...
    initPeople: function() {
        ...
        var addList = [
            {sound:res.see1_sound1},{sound:res.see2_sound3},{sound:res.see1_sound7},
            {sound:res.see2_sound2},{sound:res.see2_sound1},{sound:res.see2_sound4},
