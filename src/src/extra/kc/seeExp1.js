var seeExp1 = myLayer.extend({
    ...
    initUI:function(){
        ...
        var node = loadNode(res.kc_seeExp1_json, uiList)
        ...
        var judgeList = []
        var wenziList = []
        var local = [6,7,5,4,3,3,3]
        for(var i = 0 ; i < 7 ; i++){
            wenziList[i] = null
            var name = sprintf("#1_judge_%d.png", i+1)
        ...
        createSp("#see1_title.png",cc.p(600,590),self)
        self.nodebs.show()
        node.curIndex = 10
        for(var i = 0 ; i < 7 ; i++){
        ...
        node.callFun = function(index){
            ...
            for(var i = 0 ; i < 7 ; i++){
                ...
                if(index == i){
                    ...
                    if(!select){
                        var name = sprintf("#1_select_%d.png", i+1)
                        selectList[i] = createSp(name,node[uiList[7+i]].getPosition(),self)
                    }
                    if(!wenzi){
                        var name = sprintf("#1_wenzi_%d.png", i+1)
                        wenziList[i] = createSp(name,cc.p(920,370),self)
                    }
    ...
    initPeople: function() {
        ...
        var addList = [
            {sound:res.see1_sound1},{sound:res.see1_sound2},{sound:res.see1_sound3},
            {sound:res.see1_sound4},{sound:res.see1_sound5},{sound:res.see1_sound6},
            {sound:res.see1_sound7}
        ]
