var learnLayer = myLayer.extend({
    ...
    initUI:function(){
        ...
        var uilist = [
            "btn_home", "btn_answer", "btn_upload", "btn_retry", "judge1", "judge2", "judge3",
            "btn_help"
        ]
        var bg = loadNode(res.yddfs_learn_json, uilist, "bg")
        bg.setPosition(getMiddle(0, 20))
        safeAdd(self.inside_node, bg)

        var localZOrder = 20  //对放大的图片设置显示优先级
        var curBigItem = []   //用于存放当前的大图片，用于关闭回收
        var judge1 = bg.judge1//此处添加
            ...
            if (judge.list) {
                ...
                if (judgeInfo) {
                    ...
                    for (var i = 0; i < list.length; i++) {
                        if (list[i]) {
                            var item = list[i]
                            list[i].setPosition(cc.p(70 + count % nums * devidex, 70))
                            safeAdd(judge, item)
                            count++
                        }
                    }
        ...
        var createBtnFun = function(item){
            ...
            btn_add.setPosition(item.width-15, 15)
        ...
        var showList = createList({
            scale: 1,
            list: self.getRand(),
            pos: getMiddle(350, -20),
            num: 4,
            size: cc.size(140, 510),
            mix: 20,
            arrow: "white",
            color: "yellow",
            imgScale: 1.2,
        ...
        bg.btn_upload.addClickEventListener(function(){
            ...
            var three = judge3.judgeAnswer()//此处添加
    ...
    getRand: function() {
        ...
        for (var i = 0; i < 12; i++) {
            list.push(sprintf("#smallImg%02d.png", rand[i]))
            bigList.push(sprintf("#bigImg%02d.png", i))
        }
        return list
    },
